...
fdir = r'C:\Users\Michael\Analysis\myRecordings_extra\21-07-21\\'
    ...
    if 'slice5_merged' in fname:
        ...
        for elec in elecs2plot:
            if filtered:
                ...
                plt.savefig(
                    r'C:\Users\Michael\Analysis\myRecordings_extra\21-07-21\Figures\Spectrograms\slice5_merged.h5\gamma\raw\\' + elec + '_raw.png')
                plt.close()
                reader.electrodes_data[elec] = butter_lowpass_filter(reader.electrodes_data[elec], Fs_raw, 200)
                plot_spectrogram(reader.electrodes_data[elec], Fs_raw)
                plt.savefig(
                    r'C:\Users\Michael\Analysis\myRecordings_extra\21-07-21\Figures\Spectrograms\slice5_merged.h5\gamma\raw\\' + elec + '_lowpass.png')
                plt.close()
                sig_length = float(reader.electrodes_data['A4'].shape[0] / Fs_raw)  # get signal length in seconds
                num_samples = int(sig_length * 1000)
                from scipy import signal
                reader.electrodes_data[elec] = signal.resample(reader.electrodes_data[elec], num_samples)
                plot_spectrogram(reader.electrodes_data[elec], Fs)
                plt.savefig(
                    r'C:\Users\Michael\Analysis\myRecordings_extra\21-07-21\Figures\Spectrograms\slice5_merged.h5\gamma\raw\\' + elec + '_resample.png')
                plt.close()
                plot_spectrogram(reader.electrodes_data[elec], Fs)
                reader.electrodes_data[elec] = butter_highpass_filter(reader.electrodes_data[elec], 1000, 1)
                plt.savefig(
                    r'C:\Users\Michael\Analysis\myRecordings_extra\21-07-21\Figures\Spectrograms\slice5_merged.h5\gamma\raw\\' + elec + '_highpass.png')
                plt.close()
                plot_spectrogram(reader.electrodes_data[elec], Fs)
                reader.electrodes_data[elec] = iir_notch(reader.electrodes_data[elec], 1000 * pq.Hz, frequency=60 * pq.Hz,
                                                         quality=60., axis=-1)
                plt.savefig(
                    r'C:\Users\Michael\Analysis\myRecordings_extra\21-07-21\Figures\Spectrograms\slice5_merged.h5\gamma\raw\\' + elec + '_notch.png')
                plt.close()
