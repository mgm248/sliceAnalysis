...
segment = [-1, 6]
ffolder = r'C:\Users\Michael\Analysis\myRecordings_extra\21-07-01\\'
fname = 'slice8_merged.h5'
rec_fname = '2021-07-01T15-34-23McsRecording'
...
for unit in range(0, len(all_spikes)):
   # if cluster_info[unit + 1][5] == 'good':
    ...
    tstop = exp_df['t_start'].iloc[-1]+10*s
    if unitST[-1] > tstop:
        tstop = unitST[-1] + 2
    ...
    for row in range(0,exp_df.shape[0]):
        start = exp_df.loc[row, 't_start']+segment[0]*s
        end = exp_df.loc[row, 't_start']+segment[1]*s
        try:
            seg_st = neo_st.time_slice(start, (end - .001*s) + .001 * pq.s)
            d = {
                'event spike rate': np.squeeze(seg_st),
                'description': exp_df.loc[row, 'frequency'],
                'Unit #': unit,
                'Event time': float(exp_df.loc[row, 't_start'] / s)
            }
            all_d.append(d)
        except ValueError:
...
frequency = 2
curr_df = ev_sr_df[ev_sr_df['description']==frequency]
goodunits = [4, 10, 15, 17, 21, 30, 35, 38, 45, 46, 54, 60, 62, 67, 69, 75, 84, 85, 92, 93, 100, 108, 112, 115, 116, 121, 122]
for unit in goodunits:
    ...
    plt.xlim(0,.2)
    plt.savefig(r'C:\Users\Michael\Analysis\myRecordings_extra\21-07-01\Figures\spikes\probe_frequency\perneuron\2Hz\rasters\\'+str(unit)+'_zoomedin.png')


def compare_conditions(comparison, units, shape):
    comparisons = {'delays': ['A delay B', 'B delay A'], 'varyA': ['A medium '+shape, 'B medium '+shape, 'A strong B medium '+shape, 'A weak B medium '+shape],
                   'varyB': ['A medium '+shape, 'B medium '+shape, 'A medium B strong '+shape, 'A medium B weak '+shape]}
