...
rec_fname = '2021-02-04T14-09-55McsRecording'
...
data_saveloc = 'D:\myRecordings\Analysis\spike_analysis\\test\\' #specify a folder where you'll save
Fs=20000 #I just hardcode to avoid having to load the file data through ceed
reader = CeedDataReader(ceed_data)
# open the data file
reader.open_h5()
n_trials = 20
segment = [0,1] #relative to stimulus, when do you want the segment to start and stop
...
for exp in range(0, len(reader.experiments_in_file)):
    ...
    # if reader.experiment_stage_name == 'enclosed blue':
        ...
        if reader.electrode_intensity_alignment is not None:
            spike_counts = np.empty((len(all_spikes), 3, n_trials)) #where 3 is the number of conditions (stimulus types)
            spike_counts[:] = np.nan
            ic0 = -1
            ic1 = -1
            ic2 = -1
                    ...
                    for unit in range(0, len(all_spikes)):
                        ...
                        if cluster_info[unit+1][5]=='good':
                            ...
                            neo_st.append(neo.core.SpikeTrain(unitST, units=s, t_start=0 * s,
                                                              t_stop=unitST[-1] + 5))
                            spike_counts[unit, condition, c_idx], x\
