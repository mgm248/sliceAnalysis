...
# epochs_data = np.empty((seg_ob.shape[0], 2, int(segment_length * int(rfs))))
# epochs_data[:, 0, :] = seg_ob
# epochs_data[:, 1, :] = seg_pcx
...
plt.subplot(1, 4, 3)
plt.imshow(con[1, 0, :, :], extent=[-.5, 1, freqs[0], freqs[-1]],
           aspect='auto', origin='lower')
plt.title('Coh raw')

plt.subplot(1, 4, 4)
plt.imshow(con_norm[:, :].T, extent=[-.5, 1, freqs[0], freqs[-1]],
           aspect='auto', origin='lower')
plt.title('Coh norm')
