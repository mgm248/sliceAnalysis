...
for fname in os.listdir(ffolder):
    if '_merged' in fname:
        slicedir = 'D:\myRecordings\\1_22_20\Figures\\tf\\'+fname+'\\'
        try:
            os.mkdir(slicedir)
        except:
        ...
        reader.electrodes_data = np.load(ffolder+'filt_and_rsamp\\' + fname.replace('.h5','') +'.npy',allow_pickle=True)
        ...
        for experiment in experiments:
            ...
            epochs = mne.Epochs(raw, events, event_ids[experiment], tmin=segment[0], tmax=segment[1], event_repeated='drop', baseline=None)
