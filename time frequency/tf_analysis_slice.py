...
epochs = mne.Epochs(raw, events, event_ids['Experiment Stage ABC full cosine ShapeA'], tmin=segment[0], tmax=segment[1], event_repeated='drop', baseline=None)
...
plt.close()
fig = plt.imshow(np.mean(tmp_itc.data,(0)),
                 extent=[tmp_pow.times[0], tmp_pow.times[-1], tmp_pow.freqs[0], tmp_pow.freqs[-1]],
                 aspect='auto', origin='lower', interpolation='none')#, vmin=-.2, vmax=.2)
...
for i in range(tmp_pow.data.shape[0]):
    ...
    plt.savefig('D:\\myRecordings\\2-18-21\Figures\\tf_analysis\slice7_perelec_itc\\' + elecs2plot[i])
